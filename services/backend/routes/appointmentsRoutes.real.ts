...
export function createAppointmentsRoutesReal(db: Database): Router {
  ...
  router.post('/appointments', async (req: Request, res: Response) => {
    try {
      ...
      const fields = ['date', 'time', 'duration', 'notes', 'status'];
      const values = [date, time, duration, notes, status];
      ...
      const result = await db.query(
        `INSERT INTO appointments (${fields.join(', ')}) VALUES (${placeholders.join(', ')}) RETURNING *`,
        values
      );
