...
export function createQuotationsRoutesReal(db: Database): Router {
  const router = Router();
  ...
  router.get('/quotations', async (req: Request, res: Response) => {
    try {
      ...
      logger.error('Failed to get quotations', (error as Error).message);
  ...
  router.get('/quotations/:id', async (req: Request, res: Response) => {
    try {
      ...
      logger.error('Failed to get quotation', (error as Error).message);
  ...
  router.post('/quotations', async (req: Request, res: Response) => {
    try {
      ...
      // Inserir cotação sem client_id (campo não existe na tabela)
      const result = await db.query(
        'INSERT INTO quotations (client_name, client_email, client_phone, client_address, services, total, status, valid_until, notes) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING *',
        [client_name, client_email, client_phone, client_address, JSON.stringify(services), parseFloat(total), status, valid_until, notes]
      );

      return res.status(201).json({ success: true, data: (result as any).rows[0], message: 'Quotation created successfully' });
    } catch (error) {
      logger.error('Failed to create quotation', (error as Error).message);
  ...
  router.put('/quotations/:id', async (req: Request, res: Response) => {
    try {
      ...
      if (clientEmail !== undefined) { fields.push(`client_email = $${idx++}`); values.push(clientEmail); }
      if (clientName !== undefined) { fields.push(`client_name = $${idx++}`); values.push(clientName); }
      if (clientPhone !== undefined) { fields.push(`client_phone = $${idx++}`); values.push(clientPhone); }
      if (clientAddress !== undefined) { fields.push(`client_address = $${idx++}`); values.push(clientAddress); }
      ...
      return res.json({ success: true, data: (result as any).rows[0], message: 'Quotation updated successfully' });
    } catch (error) {
      logger.error('Failed to update quotation', (error as Error).message);
  ...
  router.delete('/quotations/:id', async (req: Request, res: Response) => {
    try {
      ...
      logger.error('Failed to delete quotation', (error as Error).message);
