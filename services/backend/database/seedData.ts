...
export class DataSeeder {
  ...
  async hasData(): Promise<boolean> {
    try {
      ...
      logger.error('Error checking existing data:', (error as Error).message);
      return false;
    }
  }
  ...
  async seedDefaultData(): Promise<void> {
    try {
      ...
      logger.error('Failed to seed default data:', (error as Error).message);
      throw error;
    }
  }
  ...
  private async seedEmailTemplates(): Promise<void> {
    const defaultTemplates: DefaultEmailTemplate[] = [
      {
        id: 'orcamento-padrao',
...
Equipe Portal Services`,
  ...
  private async seedSystemSettings(): Promise<void> {
    const defaultSettings: DefaultSystemSetting[] = [
      {
        id: 'email-config',
        key: 'email_config',
  ...
  async forceReseed(): Promise<void> {
    try {
      ...
      logger.error('Failed to force reseed:', (error as Error).message);
      throw error;
    }
  }
}
