...
const createServiceSchema = z.object({
  ...
  unit: z.string().max(50).default('hour'),
...
export function createServicesRoutes(db: Database): Router {
  const router = Router();
   ...
   */
  router.post('/', validateBody(createServiceSchema), async (req: Request, res: Response) => {
    try {
      const { name, description, category, price, unit, estimated_time, materials, active } = req.body;
      const id = uuidv4();

      const result = await db.query(
        `INSERT INTO services (id, name, description, category, price, unit, estimated_time, materials, active) 
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) 
         RETURNING id, name, description, category, price, unit, estimated_time, materials, active, created_at, updated_at`,
        [id, name, description, category, price, unit, estimated_time, JSON.stringify(materials), active]
      );
   ...
   */
  router.put('/:id', validateBody(updateServiceSchema), async (req: Request, res: Response) => {
    try {
      const { id } = req.params;
