...
export function createClientsRoutesReal(db: Database): Router {
  const router = Router();
  ...
  router.get('/clients/:id', async (req: Request, res: Response) => {
    try {
      ...
      logger.error('Failed to get client', (error as Error).message);
  ...
  router.post('/clients', async (req: Request, res: Response) => {
    try {
      ...
      return res.status(201).json({ success: true, data: (result as any).rows[0], message: 'Client created successfully' });
    } catch (error) {
  ...
  router.put('/clients/:id', async (req: Request, res: Response) => {
    try {
      ...
      return res.json({ success: true, data: (result as any).rows[0], message: 'Client updated successfully' });
    } catch (error) {
