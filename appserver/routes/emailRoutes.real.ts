...
const logger = createLogger('EmailRoutesReal');

export function createEmailRoutesReal(db: Database): Router {
  const router = Router();
  ...
  router.get('/emails/:id', async (req: Request, res: Response) => {
    try {
      ...
      logger.error('Failed to get email', (error as Error).message);
  ...
  router.post('/emails', async (req: Request, res: Response) => {
    try {
      ...
      logger.error('Failed to create email', (error as Error).message);
      return res.status(500).json({ success: false, error: 'Failed to create email' });
    }
  });

  // Update email
  router.put('/emails/:id', async (req: Request, res: Response) => {
    try {
      const id = parseInt(req.params.id);
      if (isNaN(id)) {
        return res.status(400).json({ success: false, error: 'Invalid email ID' });
      ...
      if (snippet !== undefined) { fields.push(`snippet = $${idx++}`); values.push(snippet); }
      if (category !== undefined) { fields.push(`category = $${idx++}`); values.push(category); }
      if (confidence !== undefined) { fields.push(`confidence = $${idx++}`); values.push(confidence); }
      ...
      if (response_template !== undefined) { fields.push(`response_template = $${idx++}`); values.push(response_template); }
      ...
      logger.error('Failed to update email', (error as Error).message);
  ...
  router.delete('/emails/:id', async (req: Request, res: Response) => {
    try {
      ...
      logger.error('Failed to delete email', (error as Error).message);
  ...
  router.get('/stats', async (req: Request, res: Response) => {
    try {
      ...
      const stats = (result as any).rows.map((row: any) => ({
