...
exports.createClientsRoutes = void 0;
const express_1 = require("express");
const logger_js_1 = require("../../shared/logger.js");
const zod_1 = require("zod");
const logger = (0, logger_js_1.createLogger)('ClientsAPI');
// Validation schemas
const createClientSchema = zod_1.z.object({
    name: zod_1.z.string().min(1, 'Nome é obrigatório').max(255),
    email: zod_1.z.string().email('Email deve ter formato válido').max(255),
...
function createClientsRoutes(db) {
    ...
    router.post('/', validateBody(createClientSchema), async (req, res) => {
        try {
            ...
            const result = await db.query(`INSERT INTO clients (name, email, phone, address, notes) 
         VALUES ($1, $2, $3, $4, $5) 
         RETURNING id, name, email, phone, address, notes, created_at, updated_at`, [name, email, phone || null, address || null, notes || null]);
    ...
    router.put('/:id', validateBody(updateClientSchema), async (req, res) => {
        try {
         ...
         SET ${fields.join(', ')} 
         WHERE id = $${paramIndex} 
         RETURNING id, name, email, phone, address, notes, created_at, updated_at`, values);
            if (result.rows.length === 0) {
                return res.status(404).json({
                    success: false,
                    error: 'Cliente não encontrado'
    ...
    router.delete('/:id', async (req, res) => {
        try {
            ...
            const dependenciesCheck = await db.query(`SELECT 
          (SELECT COUNT(*) FROM quotations WHERE client_email = (SELECT email FROM clients WHERE id = $1)) as quotations,
          (SELECT COUNT(*) FROM appointments WHERE client_id = $1::text) as appointments`, [id]);
            ...
            const result = await db.query(`DELETE FROM clients 
         WHERE id = $1 
    ...
    router.get('/:id/history', async (req, res) => {
        try {
            ...
            const clientResult = await db.query('SELECT email FROM clients WHERE id = $1', [id]);
            ...
            // Buscar orçamentos
            const quotationsResult = await db.query(`SELECT id, services, subtotal, discount, total, status, valid_until, created_at 
         FROM quotations 
         WHERE client_email = $1 
         ORDER BY created_at DESC`, [clientEmail]);
            // Buscar agendamentos
            const appointmentsResult = await db.query(`SELECT id, service_ids, service_names, date, time, duration, status, created_at 
         FROM appointments 
         WHERE client_id = $1 
         ORDER BY date DESC, time DESC`, [id.toString()]);
            res.json({
                success: true,
                data: {
...
exports.createClientsRoutes = createClientsRoutes;
