...
export function createAppointmentsRoutesReal(db) {
    const router = Router();
    ...
    router.get('/appointments/:id', async (req, res) => {
        try {
            ...
            logger.error('Failed to get appointment', error.message);
    ...
    router.post('/appointments', async (req, res) => {
        try {
            ...
            return res.status(201).json({ success: true, data: result.rows[0], message: 'Appointment created successfully' });
        }
        catch (error) {
    ...
    router.put('/appointments/:id', async (req, res) => {
        try {
            ...
            if (serviceIds !== undefined) {
                ...
                values.push(JSON.stringify(serviceIds));
            }
            if (serviceNames !== undefined) {
                fields.push(`service_names = $${idx++}`);
                values.push(JSON.stringify(serviceNames));
            }
            if (date !== undefined) {
            ...
            const result = await db.query(`UPDATE appointments SET ${fields.join(', ')}, updated_at = CURRENT_TIMESTAMP WHERE id = $${idx} RETURNING *`, values);
            if (result.rowCount === 0) {
                return res.status(404).json({ success: false, error: 'Appointment not found' });
            }
            return res.json({ success: true, data: result.rows[0], message: 'Appointment updated successfully' });
        }
        catch (error) {
    ...
    router.put('/appointments/:id/cancel', async (req, res) => {
        try {
            const result = await db.query('UPDATE appointments SET status = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2 RETURNING *', ['cancelled', req.params.id]);
            if (result.rowCount === 0) {
                return res.status(404).json({ success: false, error: 'Appointment not found' });
            }
            return res.json({
            ...
            logger.error('Failed to cancel appointment', error.message);
...
//# sourceMappingURL=appointmentsRoutes.real.js.map