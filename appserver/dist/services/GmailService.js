...
class GmailService {
    constructor() {
        this.gmail = null;
        this.auth = null;
        this.logger = (0, logger_1.createLogger)('GmailService');
    }
    async authenticate() {
        try {
            const credentialsData = await promises_1.default.readFile('config/credentials.json', 'utf8');
            const credentials = JSON.parse(credentialsData);
            const { client_secret, client_id, redirect_uris } = credentials.web;
            const oAuth2Client = new googleapis_1.google.auth.OAuth2(client_id, client_secret, redirect_uris[0]);
            try {
                const tokenData = await promises_1.default.readFile('config/token.json', 'utf8');
                const token = JSON.parse(tokenData);
                oAuth2Client.setCredentials(token);
                oAuth2Client.on('tokens', async (tokens) => {
                    if (tokens.refresh_token) {
                        await promises_1.default.writeFile('config/token.json', JSON.stringify(tokens));
                    }
                });
            }
            catch (error) {
