...
app.put('/api/categories/:id', async (req, res) => {
  try {
    ...
    const result = await pool.query(
      'UPDATE categories SET name = $1, description = $2, color = $3, active = $4 WHERE id = $5 RETURNING *',
...
app.put('/api/services/:id', async (req, res) => {
  try {
    ...
    const result = await pool.query(
      'UPDATE services SET name = $1, description = $2, price = $3, duration = $4, category_id = $5, active = $6 WHERE id = $7 RETURNING *',
      [name, description, price, duration, category_id, active, id]
    );
    res.json({ success: true, data: result.rows[0] });
  } catch (error) {
...
app.post('/api/appointments', async (req, res) => {
  try {
    ...
    const result = await pool.query(
      'INSERT INTO appointments (title, description, start_date, end_date, client_id, service_id, status) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *',
      [title, description, start_date, end_date, client_id, service_id, status || 'scheduled']
    );
    res.status(201).json({ success: true, data: result.rows[0] });
  } catch (error) {
...
app.put('/api/appointments/:id', async (req, res) => {
  try {
    ...
    const { title, description, start_date, end_date, client_id, service_id, status } = req.body;
    const result = await pool.query(
      'UPDATE appointments SET title = $1, description = $2, start_date = $3, end_date = $4, client_id = $5, service_id = $6, status = $7 WHERE id = $8 RETURNING *',
      [title, description, start_date, end_date, client_id, service_id, status, id]
    );
    res.json({ success: true, data: result.rows[0] });
  } catch (error) {
...
app.post('/api/quotations', async (req, res) => {
  try {
    ...
    const result = await pool.query(
      'INSERT INTO quotations (title, description, total_amount, client_id, status) VALUES ($1, $2, $3, $4, $5) RETURNING *',
      [title, description, total_amount, client_id, status || 'draft']
    );
    res.status(201).json({ success: true, data: result.rows[0] });
  } catch (error) {
...
app.put('/api/quotations/:id', async (req, res) => {
  try {
    ...
    const { title, description, total_amount, client_id, status } = req.body;
    const result = await pool.query(
      'UPDATE quotations SET title = $1, description = $2, total_amount = $3, client_id = $4, status = $5 WHERE id = $6 RETURNING *',
      [title, description, total_amount, client_id, status, id]
    );
    res.json({ success: true, data: result.rows[0] });
  } catch (error) {
...
app.post('/api/emails', async (req, res) => {
  try {
    ...
    const result = await pool.query(
      'INSERT INTO emails (subject, sender, recipient, body, status) VALUES ($1, $2, $3, $4, $5) RETURNING *',
      [subject, sender, recipient, body, status || 'pending']
    );
    res.status(201).json({ success: true, data: result.rows[0] });
  } catch (error) {
...
app.put('/api/emails/:id', async (req, res) => {
  try {
    ...
    const { subject, sender, recipient, body, status } = req.body;
    const result = await pool.query(
      'UPDATE emails SET subject = $1, sender = $2, recipient = $3, body = $4, status = $5 WHERE id = $6 RETURNING *',
      [subject, sender, recipient, body, status, id]
    );
    res.json({ success: true, data: result.rows[0] });
  } catch (error) {
