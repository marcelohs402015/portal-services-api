...
import { Quotation, CreateQuotationDTO, UpdateQuotationDTO, QueryParams, ApiResponse } from '../types/entities.js';

export class QuotationRepository extends BaseRepository<Quotation, CreateQuotationDTO, UpdateQuotationDTO> {
  ...
  protected getUpdateFields(dto: UpdateQuotationDTO): string[] {
    ...
    if (dto.client_id !== undefined) fields.push('client_id');
    if (dto.status !== undefined) fields.push('status');
    if (dto.valid_until !== undefined) fields.push('valid_until');
    if (dto.notes !== undefined) fields.push('notes');
    if (dto.discount_amount !== undefined) fields.push('discount_amount');
    return fields;
  }

  protected getUpdateValues(dto: UpdateQuotationDTO): any[] {
    const values: any[] = [];
    if (dto.title !== undefined) values.push(dto.title);
    if (dto.description !== undefined) values.push(dto.description);
    if (dto.client_id !== undefined) values.push(dto.client_id);
    if (dto.status !== undefined) values.push(dto.status);
    if (dto.valid_until !== undefined) values.push(dto.valid_until);
    if (dto.notes !== undefined) values.push(dto.notes);
    if (dto.discount_amount !== undefined) values.push(dto.discount_amount);
    return values;
  }

  // Métodos específicos para orçamentos
  async findByClient(clientId: string): Promise<ApiResponse<Quotation[]>> {
    try {
      const query = `
  ...
  async findByStatus(status: string): Promise<ApiResponse<Quotation[]>> {
    try {
      const query = `
        SELECT q.*, c.name as client_name, c.email as client_email
        FROM quotations q
  ...
  async findExpired(): Promise<ApiResponse<Quotation[]>> {
    try {
      const query = `
        SELECT q.*, c.name as client_name, c.email as client_email
        FROM quotations q
  ...
  async findExpiringSoon(days: number = 7): Promise<ApiResponse<Quotation[]>> {
    try {
      const query = `
        SELECT q.*, c.name as client_name, c.email as client_email
        FROM quotations q
  ...
  async getQuotationStats(): Promise<ApiResponse<{ 
    total: number; 
    by_status: any[]; 
    total_value: number; 
  ...
  async updateStatus(id: string, status: string): Promise<ApiResponse<Quotation | null>> {
    try {
      const query = `
        UPDATE quotations 
  ...
  async markAsExpired(): Promise<ApiResponse<number>> {
    try {
      const query = `
        UPDATE quotations 
